...
// User configurable.
const GAME_KEY = window.BINJGB_GAME_KEY || 'game';
...
class VM {
  ...
  updateExtRam() {
    ...
    localStorage.setItem(GAME_KEY + '-extram', JSON.stringify(Array.from(extram)));
  }
};

const vm = new VM();

// Arranque bajo demanda (llamado desde la página). Devuelve {resumed}.
window.startBinjgb = async function(romBuffer) {
  let extRam = null;
  try {
    extRam = new Uint8Array(JSON.parse(localStorage.getItem(GAME_KEY + '-extram')));
  } catch (e) { /* sin SRAM previa */ }
...
class Emulator {
  ...
  loadState() {
    const saveStateBuffer =
        new Uint8Array(JSON.parse(localStorage.getItem(GAME_KEY + '-saveState')));
  ...
  saveState() {
    ...
    localStorage.setItem(GAME_KEY + '-saveState', JSON.stringify(Array.from(saveStateBuffer)));
    localStorage.setItem(GAME_KEY + '-saveStateTime', String(Date.now()));
  }
